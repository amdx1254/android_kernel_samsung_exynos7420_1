...
dhdpcie_dongle_attach(dhd_bus_t *bus)
{
	...
	DHD_TRACE(("%s: ENTER\n",
		__FUNCTION__));
	DHD_ERROR(("%s : ENTER ATTACH FUNTION\n", __FUNCTION__));
...
concate_revision(dhd_bus_t *bus, char *fw_path, char *nv_path)
{
	...
	DHD_ERROR(("concate_revision %d, %d, %d, %d, %d, %d\n", bus->sih->socitype, bus->sih->bustype, bus->sih->chip, bus->sih->chiprev, bus->sih->chipst, bus->sih->ccrev));
	...
	default:
		DHD_ERROR(("REVISION SPECIFIC feature is not required, %d\n", si_chipid(bus->sih)));
		return res;
	}
...
dhdpcie_bus_download_state(dhd_bus_t *bus, bool enter)
{
	...
	if (enter) {
		bus->alp_only = TRUE;
		DHD_ERROR(("concate_revision2 %d, %d, %d, %d, %d, %d\n", bus->sih->socitype, bus->sih->bustype, bus->sih->chip, bus->sih->chiprev, bus->sih->chipst, bus->sih->ccrev));
